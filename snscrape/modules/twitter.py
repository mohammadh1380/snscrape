...
class Tweet(snscrape.base.Item):
    ...
    username = snscrape.base._DeprecatedProperty('username', lambda self: getattr(self.user, 'username', None),
                                                 'user.username')
...
class _CLIGuestTokenManager(GuestTokenManager):
    ...
    def _read(self):
        ...
        validTokens = [token for token, t in o['tokens'].items() if
                       t['setTime'] >= setTimeThreshold and t.get('blockedUntil', 0) < currentTime]
        if not validTokens:
            return None
    ...
    def _write(self):
        with self._lock:
            ...
            o['tokens'] = {token: details for token, details in o['tokens'].items() if
                           details['setTime'] >= setTimeThreshold}
...
class _TwitterAPIScraper(snscrape.base.Scraper):
    ...
    def _check_api_response(self, r, apiType, instructionsPath):
        if r.status_code in (403, 404, 429):
            if r.status_code == 429 and r.headers.get('x-rate-limit-remaining',
                                                      '') == '0' and 'x-rate-limit-reset' in r.headers:
    ...
    def _make_tweet(self, tweet, user, retweetedTweet=None, quotedTweet=None, card=None, noteTweet=None, **kwargs):
        ...
        if noteTweet and 'text' in noteTweet:
            ...
            if noteTweet:
                _logger.warning(
                    f'Twitter returned an empty note tweet in tweet {tweetId}; text and entities might be incomplete')
        ...
        kwargs['conversationControlPolicy'] = ConversationControlPolicy._from_policy(
            tweet.get('conversation_control', {'policy': None})['policy'])
        return Tweet(**kwargs)
    ...
    def _make_edit_state(self, editControl):
        ...
        return EditState(
            ...
            editableUntilDate=datetime.datetime.fromtimestamp(int(editControl['editable_until_msecs']) / 1000,
    ...
    def _graphql_timeline_tweet_item_result_to_tweet(self, result, tweetId=None, **kwargs):
        ...
        tweet = result['legacy']
        user = self._graphql_user_results_to_user(result['core']['user_results'],
                                                  userId=int(result['legacy']['user_id_str']))
                ...
                if entry['entryId'].startswith('tweet-'):
                    ...
                    if entry['content']['entryType'] == 'TimelineTimelineItem' and entry['content']['itemContent'][
                        ...
                        yield self._graphql_timeline_tweet_item_result_to_tweet(
                            entry['content']['itemContent']['tweet_results']['result'], tweetId=tweetId, **kwargs)
                    else:
                    ...
                    if entry['content']['entryType'] == 'TimelineTimelineModule':
                        for item in reversed(entry['content']['items']):
                            if not item['entryId'].startswith(
                                    entry['entryId'].split('ion-', 1)[0] + 'ion-') or '-tweet-' not in item['entryId']:
                                raise snscrape.base.ScraperException(
                                    f'Unexpected conversation entry ID: {item["entryId"]!r}')
    ...
    def _graphql_user_results_to_user(self, results, userId=None):
        ...
        if 'profile_image_shape' in results['result']:
            kwargs['profileImageShape'] = ProfileImageShape._from_twitter_string(
                results['result']['profile_image_shape'])
        return self._user_to_user(results['result']['legacy'],
                                  id_=userId if userId is not None else int(results['result']['rest_id']), **kwargs)
...
class TwitterSearchScraper(_TwitterAPIScraper):
    ...
    def __init__(self, query, redis_obj, *, cursor=None, mode=TwitterSearchScraperMode.LIVE, top=None, maxEmptyPages=1, **kwargs):
    ...
    def _cli_setup_parser(cls, subparser):
        ...
        subparser.add_argument('query', type=snscrape.utils.nonempty_string_arg('query'),
...
class TwitterUserScraper(TwitterSearchScraper):
    ...
    def _get_entity(self):
        ...
        obj = self._get_api_data(endpoint, _TwitterAPIType.GRAPHQL,
                                 params={'variables': variables, 'features': features},
                                 instructionsPath=['data', 'user'], account=self.account)
    ...
    def get_items(self):
        ...
        for obj in self._iter_api_data('https://twitter.com/i/api/graphql/fn9oRltM1N4thkh5CVusPg/UserTweetsAndReplies',
                                       ...
                                       instructionsPath=['data', 'user', 'result', 'timeline_v2', 'timeline',
                                                         'instructions'], account=self.account):
            if not obj['data'] or 'result' not in obj['data']['user']:
                raise snscrape.base.ScraperException('Empty response')
            if obj['data']['user']['result']['__typename'] == 'UserUnavailable':
                raise snscrape.base.EntityUnavailable('User unavailable')
...
class TwitterHashtagScraper(TwitterSearchScraper):
    ...
    @classmethod
    def _cli_setup_parser(cls, subparser):
        subparser.add_argument('hashtag', type=snscrape.utils.nonempty_string_arg('hashtag'),
...
class TwitterCashtagScraper(TwitterSearchScraper):
    ...
    @classmethod
    def _cli_setup_parser(cls, subparser):
        subparser.add_argument('cashtag', type=snscrape.utils.nonempty_string_arg('cashtag'),
...
class TwitterTweetScraper(_TwitterAPIScraper):
    ...
    def get_items(self):
        ...
        if self._mode is TwitterTweetScraperMode.SINGLE:
            ...
            for obj in self._iter_api_data(url, _TwitterAPIType.GRAPHQL, params, paginationParams,
                                           direction=_ScrollDirection.BOTH, instructionsPath=instructionsPath):
                if not obj['data']:
                    continue
                yield from self._graphql_timeline_instructions_to_tweets(
            ...
            while queue:
                ...
                for obj in self._iter_api_data(url, _TwitterAPIType.GRAPHQL, thisParams, thisPagParams,
                                               direction=_ScrollDirection.BOTH, instructionsPath=instructionsPath, account=self.account):
                    if not obj['data']:
                        continue
    ...
    def _has_moderated_replies(self, obj, tweetId):
        for instruction in obj['data']['threaded_conversation_with_injections_v2']['instructions']:
            if instruction['type'] != 'TimelineAddEntries':
                continue
            for entry in instruction['entries']:
                if entry['entryId'] == f'tweet-{tweetId}' and entry['content'][
                    'entryType'] == 'TimelineTimelineItem' and entry['content']['itemContent'][
                    'itemType'] == 'TimelineTweet':
    ...
    def _get_moderated_replies(self, tweetId):
        ...
        for obj in self._iter_api_data(url, _TwitterAPIType.GRAPHQL, params, paginationParams,
                                       direction=_ScrollDirection.BOTH, instructionsPath=instructionsPath):
            yield from self._graphql_timeline_instructions_to_tweets(
                obj['data']['tweet']['result']['timeline_response']['timeline']['instructions'],
                includeConversationThreads=True)
...
class TwitterListPostsScraper(TwitterSearchScraper):
    ...
    @classmethod
    def _cli_setup_parser(cls, subparser):
        subparser.add_argument('list', type=snscrape.utils.nonempty_string_arg('list'),
...
class TwitterCommunityScraper(_TwitterAPIScraper):
    ...
    def _get_entity(self):
        ...
        obj = self._get_api_data('https://twitter.com/i/api/graphql/bC3Saf4niY6YuzJWV2oUGg/CommunitiesFetchOneQuery',
                                 _TwitterAPIType.GRAPHQL, params=params, instructionsPath=['data', 'communityResults'])
    ...
    def get_items(self):
        ...
        for obj in self._iter_api_data(
                ...
                _TwitterAPIType.GRAPHQL, params, paginationParams,
                instructionsPath=['data', 'communityResults', 'result', 'community_timeline', 'timeline',
                                  'instructions']):
            if obj['data']['communityResults']['result']['__typename'] == 'CommunityUnavailable':
                raise snscrape.base.EntityUnavailable('Community unavailable')
            yield from self._graphql_timeline_instructions_to_tweets(
...
class TwitterUsersScraper(_TwitterAPIScraper):
    ...
    def get_items(self):
        ...
        obj = self._get_api_data('https://twitter.com/i/api/graphql/GD4q8bBE2i6cqWw2iT74Gg/UsersByRestIds',
                                 _TwitterAPIType.GRAPHQL, params={'variables': variables, 'features': features},
                                 instructionsPath=['data', 'users'])
